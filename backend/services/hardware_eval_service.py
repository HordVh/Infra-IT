...
def _ai_assessment(model, candidate: dict, inventory_avg: dict) -> str:
    """
    Call Gemini to produce a concise 2-3 sentence functional fit assessment
    for a candidate device vs. the current inventory average.
    """
    if model is None:
        return (
...
def evaluate_hardware(candidate_df: pd.DataFrame, inventory_df: pd.DataFrame) -> list[dict]:
    ...
    for _, row in candidate_df.iterrows():
        ...
        ai_text = _ai_assessment(gemini, candidate_context, inventory_avg)
